# Purpose: Get-SystemsReport — McAfee ePolicy Orchestrator reporting.
Function Get-SystemsReport {
...
$server = "199.253.142.237" #enter your own SMTP server DNS name / IP address here
$list = Get-Content -Path "C:\Temp\list.txt" #This accepts the argument you add to your scheduled task for the list of servers. i.e. list.txt
...
foreach ($computer in $computers) {
  ...
  Create-PieChart -FileName ((Get-Location).Path + "C:\Temp\chart-$computer") $FreeRAM, $UsedRAM
...
$HTMLmessage | Out-File ((Get-Location).Path + "C:Temp\report.html")
