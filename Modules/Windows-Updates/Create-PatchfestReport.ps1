# Purpose: Create-PatchfestReport — Windows Update and patch management.
Function Create-PatchfestReport {
<#PSScriptInfo 
...
$cert = ( Get-ChildItem -Path Cert:\LocalMachine\My\E291EF5BAA23F6F7BB118A3B50D97C367D8F40D4 )

Start-UdDashboard -Content {
    New-UdDashboard -Title "PatchFest Reporting Dashboard" -Color '#FF050F7F' -Content {
      New-UDElement -Tag "h1" -Content { "Super Secure" }
        New-UdRow {
            ...
            New-UdColumn -Size 6 -Content {
                ...
                New-UdRow {
                    New-UdColumn -Size 12 {
                        New-UdGrid -Title "Windows Updates" -Headers @("Source", "Description", "HotFixID", "InstalledBy", "InstalledOn") -Properties @("Source", "Description", "HotFixID", "InstalledBy", "InstalledOn") -AutoRefresh -RefreshInterval 60 -Endpoint {
                            Get-Hotfix -ComputerName | Out-UDGridData
                        }
                      }
                    }
                  }
