# Purpose: Push-PatchToSNMFSS001 — Windows Update and patch management.
#----------------------------
...
#
#Usage: .\ pushPatch.ps1 <hostnames> ex: pushPatch.ps1 SJRDITS01 or pushPatch.ps1 SJRDITS01,214.193.3.93,248.85.59.40
#Also recommended usage: Get-Content hostnames.txt | ForEach-Object { pushPatch.ps1 $_}
#
#----------------------------
function Push-PatchToSNM-FSS-001 {
    param
    (
        [Parameter(Mandatory = $true, ValueFromPipeline = $true)]
        [String[]]$ComputerName,
        [switch]$CoreSatellite, #Flag to force connection to 134.127.11.250,
    ...
    Foreach ($cn in $ComputerName) {
        ...
        #if($CoreSatellite){
        #  Start-Process 'C:\Program Files (x86)\Hewlett-Packard\HPCA\Agent\radntfyc.exe' -ArgumentList "$cn radskman ip=119.223.48.126,port=3464,cat=prompt,uid=$cn,ulogon=n,hreboot=n,dname=Patch,log=connect_Patch.log,rtimeout=60,rcsuri=tcp://114.148.18.125:3464,datauri=http://114.148.18.125:3466"
        #}
        #else
        
        if ($CoreSatellite) {
            Start-Process 'C:\Program Files (x86)\Hewlett-Packard\HPCA\Agent\radntfyc.exe' -ArgumentList "$cn radskman ip=134.127.11.250 port=3464,cat=prompt,ulogon=n,hreboot=n,dname=Patch,log=connect_Patch.log,rtimeout=60,rcsuri=tcp://134.127.11.250:3464,datauri=http://134.127.11.250:3466"
        }
        else {
...
function Push-PatchToSNM-FSS-001-Parallel {
    param
    (
        [Parameter(Mandatory = $true, ValueFromPipeline = $true)]
        [String]$InputObject,
    ...
    Invoke-Parallel -InputObject (get-content($InputObject)) -ScriptBlock {
        . "E:\WindowsPowerShell\Modules\Push-PatchToSNM-FSS-001.ps1";
        if ($kill) {
            Push-PatchToSNM-FSS-001 $_ -kill
        }
        else {
            Push-PatchToSNM-FSS-001 $_
        }
