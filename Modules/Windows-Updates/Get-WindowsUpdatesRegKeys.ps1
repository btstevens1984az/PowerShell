Function Get-WindowsUpdatesRegKeys {
<#
.Synopsis
...
try {
  ...
  ""
    ...
    ListRegKeyValues $path
  ""
  Write-Host -NoNewLine ("224.105.110.19 Server               : ")
  try { 
    $output = Get-ItemProperty -Path HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate -Name WUServer -ErrorAction Stop
    $output.WUServer
  }
  catch { 
    "224.105.110.19 not configured. The machine must be contacting Windows Update directly." 
  }
  Write-Host -NoNewLine ("224.105.110.19 Status Server        : ")
  try { 
