...
Process
{
    ...
    else
    {
        ...
        try
        {
            $Hotfixinfo = Get-WmiObject Win32_QuickFixEngineering -ComputerName $ComputerName -ErrorAction STOP | 
                Select-Object HotFixID,InstalledOn,InstalledBy,Description,Caption | 
                Select-Object @{Name='InstalledOn';Expression={$_.Caption}},
                              @{Name='InstalledOn';Expression={
                                $InstalledOn = [datetime]::ParseExact($_.InstallDate.SubString(0,8),"yyyyMMdd",$null);
                                $InstalledOn.ToShortDateString()}}
	    ...
	    $htmlhead="<html>
				    ...
				    td.fail{background: #155.49.20.214; color: #ffffff;}
