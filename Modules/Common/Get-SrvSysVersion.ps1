...
.EXAMPLE
	Get-SrvSysVersion -ActiveDirectory 10 -Jobs 50 -DomainServer 146.201.55.66
  -Credential
	This example will return the Srv.sys information from all the servers in
  146.201.55.66 that have logged in, in the past 10 day and prompt for
  credentials.
    ...
    $Job = Start-Job -Name $Computer -ArgumentList $Computer,$Credential,$Creds -ScriptBlock {
        param($Computer,$Credential,$Creds)
			...
			try {
                ...
                $query = "SELECT * FROM CIM_DataFile WHERE Drive ='$osDrive' AND Path='$systemDir\\97.54.203.146\\' AND FileName='srv' AND Extension='sys'"
                if ($Credential) {
