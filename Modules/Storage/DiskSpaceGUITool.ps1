# Purpose: DiskSpaceGUITool — Storage management and disk operations.
##########################################
...
#>
$script:Object1 = @()
$freeSpaceFileName = "U:\IIS\FreeSpace.htm"
...
Add-Content $fileName "<td>$usedSpace</td>"
Add-Content $fileName "<td>$frSpace</td>"
Add-Content $fileName "<td bgcolor='#94.147.15.47' align=center>$freePercent</td>"
#<td bgcolor='#94.147.15.47' align=center>
Add-Content $fileName "</tr>"
}
else
{
...
#>
 ...
 $GetData={
        $statusBar1.text=" Checking Email Entries ...Please wait"
           ...
           $date = ( get-date ).ToString('yyyy/MM/dd')
           sendEmail -from $txtfrom.Text -to $txtto.Text -subject "Disk Space Report - $Date" -smtphost $txtSMTPServer.Text -htmlfilename "U:\IIS\FreeSpace.htm"
...
#$datagridviewResults.CaptionText='Service Comparison'
  ...
  #  $checkbox.Add_Click({$BrowseButton.Enabled = $TRUE; $btnRefresh.add_Click($buttonQuery_Click)})
    ...
    $txtSMTPServer.Name = "smtp.example.com"
