# Purpose: CreateAndEnableUserFromCSV — General-purpose PowerShell utilities.
...
function funHelp()
{
 $descriptionText= `
@"
 NAME: CreateAndEnableUserFromCSV.ps1
 DESCRIPTION:
 Creates an enabled user account by reading csv file 

 PARAMETERS: 
 -csvFile name and path to the csv file containing user info
...
@"
 ...
 SYNTAX:
 CreateAndEnableUserFromCSV.ps1 

 Sample on a local machine

 CreateAndEnableUserFromCSV.ps1  -computer "Berlin"

 Creates on a remote computer named Berlin 

 CreateAndEnableUserFromCSV.ps1 -whatif
 
 Displays what if: Perform operation

 CreateAndEnableUserFromCSV.ps1 -help

 Prints the help topic for the script

 CreateAndEnableUserFromCSV.ps1 -help -full

 Prints full help topic for the script

 CreateAndEnableUserFromCSV.ps1 -help -examples

 Prints only the examples for the script

 CreateAndEnableUserFromCSV.ps1 -examples
...
$detailsText = `
@"
  The CSV file for this script will look something like the following: 
  ou, Username, SamAccount, Password
  mytestOU, euser1, euser1, Password1
  mytestOU, euser2, euser2, Password1
  Note: the top row provides the field names when using import-csv
"@ #end detailsText

  if($examples) { $examplesText; $remarks; exit }
  if($full)     { $descriptionText; $examplesText; $detailsText; exit } 
...
Function funWhatIf()
{
 "what if: Perform operation create user from csv file. The file is:
  $csvFile."
 exit
} #end funWhatIf

Function funCreateUsers()
{
 if(!(Test-Path $csvFile)) 
  { 
   $(Throw 'Unable to find $csvFile. 
     Try this: $($MyInvocation.ScriptName) -help ?')
  }
 $aryUser= import-csv -Path $csvFile
 $Class = "User"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUqBU/uYq9AwAZtH75/e+IMfm0
# NsSggh7hMIIEEjCCAvqgAwIBAgIPAMEAizw8iBHRPvZj7N9AMA0GCSqGSIb3DQEB
...
# AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUieYLm2aTKHkDVCD8
# 2pFuqUKpujowXgYKKwYBBAGCNwIBDDFQME6gJoAkAE0AaQBjAHIAbwBzAG8AZgB0
# ACAATABlAGEAcgBuAGkAbgBnoSSAImh0dHA6Ly93d3cubWljcm9zb2Z0LmNvbS9s
# ZWFybmluZyAwDQYJKoZIhvcNAQEBBQAEggEAY2DmY4asljwuT3HVCvRUiU/i0mN8
# xeygy+awhuc8PSGNWS0goipMjmHAKSfurRr1jQol2Xkr3YuBOWrhojw1LZ6c5eY9
# smevGv8RxMmX1qVlVAQEfGd76KymcxEHa81buTVbtxf1ysXR8o8eBqa1K7aj9xkM
# RGTFYSEvebv6s19f6WOw1dGSptmkhS+qCVF7CCzcWMII8H9ve56nD/+zPvEaRmCb
# vCvfXld+60iTgIxbrDWjjS43O97Z+JXECwr51uZ9I2hzpDhHI7KL2jEcGCPHsOcE
# vUQNkttpy4I4uprpENElcv5UgBWXa6AjP/zhwhvBa/vTiFK6kNxBdF56PaGCAh8w
...
# hkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTA4MDIwODIwMDM0NVowIwYJKoZIhvcN
# AQkEMRYEFFPGJpRYW2KY3wtD/Tzv9m8iXp2YMA0GCSqGSIb3DQEBBQUABIIBAK8o
# +lnFGHDxQVUFCTAHZGul4Fb70nayfly1MOiD+kxQOJ2ZaBh9GFUWtiSFfK+bj9YF
# upLFZZS2BhaFo61gpYo4g1r+IJTdnkn7lOTmPNC6XAOq3/QzHsvlHQ1qDKFgsd8E
# oHkt5NgaXKfQPoo1JHoo+db5CVlpP/I4XxAMVkhRZ5uAMCsUJg7w6S0etH4DuSkV
# y67Imi1LiVBCBxx2Jqdn1d1fbMWbQyDm/O2JV8Xs5jbiiuauxM5Laptye6j1ne9F
# GqDgMeklDUbAgwkVql2o1vk8i/BIMPmOfdpTBhzUUp9PYwLhoQAXZwZ4dnWcWrlG
# DzCU7KUwq6ELu7xh6cU=
