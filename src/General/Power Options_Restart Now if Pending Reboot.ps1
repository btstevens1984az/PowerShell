# Purpose: Power Options Restart Now if Pending Reboot — General-purpose PowerShell utilities.
...
if ($pendingreboot -eq $true) {
	Append-Richtextbox -ComputerName $computername -Source $scriptbasename -Message "Reiniciando $computername" -logfile 'ps1command.log'
	...
	Append-Richtextbox -ComputerName $computername -Source $scriptbasename -Message "No es necesario reiniciar $computername" -MessageColor $css.richtextcolorOK -logfile 'ps1command.log'
}