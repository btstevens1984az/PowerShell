# Purpose: 12 shouldcontinue 1 — General-purpose PowerShell utilities.
function New-EmployeeFile
{
    ...
    Process
    {
        ...
        {
            if ($StartDate -le (Get-Date) -or $PSCmdlet.ShouldContinue("Start date for $Username is in the future, should this record really be saved?", "Are you sure?"))
            {
                ...
                $employeeData | Export-CSV .\emp-e-$username.csv -NoTypeInformation -Confirm:$false
            }
        }
    }
}
...
# Bad user data can be handled with an additional ShouldContinue prompt
