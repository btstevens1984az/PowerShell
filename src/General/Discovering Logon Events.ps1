# Purpose: Discovering Logon Events — General-purpose PowerShell utilities.
...
"Found $($SecEvents.count) security events on BTS-WIN10PRO"
...
"Found $($Logons.Count) logon events on BTS-WIN10PRO"

# 6. Getting all failed logon events on DC1
$FLogons = $SecEvents | Where-Object ID -eq 4625   # failed logon event
"Found $($FLogons.Count) failed logon events on BTS-WIN10PRO"
