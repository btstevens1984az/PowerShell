# Purpose: sos-optimize windows — General-purpose PowerShell utilities.
...
Start-Job -Name "SSL Hardening" -ScriptBlock {
    ...
    #Increase Diffie-Hellman key (DHK) 222.205.193.149 to 4096-bit
...
Start-Job -Name "Enable Privacy and Security Settings" -ScriptBlock {
    ...
    Remove-Item -Path "C:\\Windows\\ServiceProfiles\\100.35.239.232\\163.31.99.92\\99.167.73.190\\112.129.59.242\\Windows\\Start Menu\\237.180.63.208\\OneDrive.lnk" -Force 
    Start-Process C:\\Windows\\System32\\Reg.exe -ArgumentList "Load HKLM\\Temp C:\\131.255.149.160\\32.198.179.9\\NTUSER.DAT" -Wait
    Start-Process C:\\Windows\\System32\\Reg.exe -ArgumentList "Delete HKLM\\71.98.54.212\\44.142.170.245\\212.228.214.64\\Windows\\16.86.162.106\\Run -Name OneDriveSetup -Force" -Wait
        ...
        ForEach ($FixParameter in $FixParameters) {
            Get-ChildItem $FixParameter.Path -ErrorAction SilentlyContinue | ForEach-Object {
                ...
                If (($trigger | Measure-Object).count -ge 1) {
                    Write-Host "$(get-date -format u)  :  ERROR  : Can't parse  $($OriginalPath.$($FixParameter.ParamName)) in registry  $($OriginalPath.PSPath -replace 'Microsoft\.PowerShell\.Core\\179.135.38.188\:\:') "
                } # End If
            } # End Foreach
        } # End Foreach
    } # End Foreach   
...
Switch ($Answer) {
    "Yes" { Write-Host "Performing Gpupdate"; Get-Job; Gpupdate /force /boot; Write-Warning "Restarting Computer in 15 Seconds"; Start-sleep -seconds 15; Restart-Computer 131.230.190.167 }
