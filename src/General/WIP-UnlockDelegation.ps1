# Purpose: WIP-UnlockDelegation — General-purpose PowerShell utilities.
...
function Call-Unlocker_pff {
    ...
    $buttonCheck_Click={
		...
		$expirationdate = ($searcher.findAll().properties).'msds-userpasswordexpirytimecomputed'
		$expdate = [datetime]::FromFileTime([string]$expirationdate) | get-date -format "MM/dd/yy hh:mm tt"
        $Results = $Searcher.FindAll()
		# Get the current date and time
        $DateFormat = Get-Date -Format "yyyy/MM/dd-HH:mm:ss"
		
		$ConnectionTest = Test-NetConnection dc-contoso-com -hops 2
		
		if ($ConnectionTest.PingSucceeded -eq $False) {
			Load-ListBox `
			-ListBox $listBox1 `
			-Items "$DateFormat - Cannot reach server, activate VPN if remote" `
			-Append
			}
			else {
        ...
        if ($Results -ne $null){
            ...
            if($Results.properties.lockouttime -gt 0){
                ...
                Load-ListBox `
					-ListBox $listBox1 `
					-Items "$DateFormat - $name - Account Locked Out `n Password expires: $expdate" `
					-Append
            }else{
                # Show the information in the ListBox
                Load-ListBox `
					-ListBox $listBox1 `
					-Items "$DateFormat - $name - Account NOT Locked Out! `n Password expires $expdate" `
					-Append}
        }else {
            # Show the information in the ListBox
            Load-ListBox `
    ...
    $formUnlocker.ClientSize = '750, 275'
    $formUnlocker.MaximumSize = '750, 314'
    $formUnlocker.Name = "formUnlocker"
    $formUnlocker.Text = "AD Account Unlock Tool"
    ...
    $statusbar1.Text = "Brock & Scott, PLLC"
    #
    # listbox1
    #
