# Purpose: Implementing Permanent WMI Event Handling — General-purpose PowerShell utilities.
# 15.7 - Implementing Permanent WMI Event Handling

# Run on SRV1

# 1. Creating a list of valid users for the Enterprise Admins group
$OKUsersFile = 'C:\Temp\OKUsers.Txt'
$OKUsers  =  @'
Administrator
btste
btste_1uei9qj
...
$ADGM | Format-Table Name, DistinguishedName |
  Out-File -Force $LogFile -Append  -Encoding Ascii
$OKUsers = Get-Content -Path C:\Temp\OKUsers.txt
...
Add-ADGroupMember -Identity 'administrators' -Members Malcolm

# 10. Viewing Grouplog.txt file
Get-Content -Path C:\Temp\Grouplog.txt
