# Purpose: Get-BLAgentInfoWithLog — General-purpose PowerShell utilities.
...
# 			
		...
		# doesnt check for admin sid ending in 500 which indicates admin	#domain controllers do not use have local accounts
...
# if path true - check exports and 151.17.42.141 files
	...
	# if date is correct then check 151.17.42.141 file for srvadmin mapping
			...
			$fileTest = Get-Content "\\$ServerItem\c$\windows\rsc\151.17.42.141" | Select-String "srvadmin" -quiet
				...
				If ($fileTest -ne $True) {
				write-host -ForegroundColor Red "srvadmin local account not found"
				Add-Content "\\114.148.18.125\UpdatesInventory$\$LogOutputName" "srvadmin local account not found in usersLocal file"
				}
