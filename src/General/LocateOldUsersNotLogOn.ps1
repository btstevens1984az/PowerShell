# Purpose: LocateOldUsersNotLogOn — General-purpose PowerShell utilities.
# NAME:LocateOldUsersNotLogOn.ps1
...
@"
 NAME: LocateOldUsersNotLogOn.ps1
 DESCRIPTION:
 Scans a local or remote domain for user accounts
 that have not logged onto the domain for an extended
 period of time which is specified in days. The default
 value of days is 30, but this can be modified by using
 the -days parameter. The default domain is the current
 domain, but this can be modified by using the -domain
 parameter. This script supports prototyping by using
 the -whatif switch. 

 PARAMETERS: 
 -domain the domain to query. Optional
 -days the number of days since logon. Optional
 -all returns all users who have not logged on within
  the time specified by -days
 -users queries for last logon information for specific
  user accounts
...
@"
 ...
 LocateOldUsersNotLogOn.ps1 -users administrator,munichadmin

 Displays last logon information of the administrator and munichadmin
 within the last 30 days

 LocateOldUsersNotLogOn.ps1 -all

 Displays last logon information of all the users from the current 
 domain within the last 30 days

 LocateOldUsersNotLogOn.ps1 -all -domain nwtraders

 Displays last logon information of all the users from the nwtraders 
 domain within the last 30 days

 LocateOldUsersNotLogOn.ps1 -whatif
 
 Displays what if: Perform operation

 LocateOldUsersNotLogOn.ps1 -help

 Prints the help topic for the script

 LocateOldUsersNotLogOn.ps1 -help -full

 Prints full help topic for the script

 LocateOldUsersNotLogOn.ps1 -help -examples

 Prints only the examples for the script

 LocateOldUsersNotLogOn.ps1 -examples
 ...
 if($users)
   {
     if($domain)
      {
       "what if: Perform operation locate users $users who have not logged on in $days days"
 ...
 if($all)
  {
   if($domain)
    {
     "what if: Perform operation locate all users who have not logged on in $days days"
     "From the $domain domain"
    }
   ELSE
    {
   ...
   foreach($user in $users)
    {
     if($user.properties.item("$attribute") -ne 0)
      ...
      ELSE { "$($user.properties.item(`"name`")) never logged on" }
    } #end foreach user
  exit
} #end funGetAllUsers
     ...
     foreach($user in $colUsers)
      {
       ...
       {
        $attributeValue = [datetime]::FromFileTime([int64]::Parse($user.properties.item($attribute)))
        If($attributeValue -ge (Get-date).addDays(-$days))
         { 
          "$($user.properties.item(`"name`")) $attributeValue"
         }
        ELSE
         {
           "$($user.properties.item(`"name`")) has not logged on in the last $days days"    
         }
       } #end if user
       ELSE { "$($user.properties.item(`"name`")) never logged on" }
      } #end foreach user
...
if(!$all -or !$users) { "missing parameter ..." ; funhelp }

# SIG # Begin signature block
# MIIkCgYJKoZIhvcNAQcCoIIj+zCCI/cCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUN1is4j452qrFFp7BIIN9UNMa
# wpSggh7hMIIEEjCCAvqgAwIBAgIPAMEAizw8iBHRPvZj7N9AMA0GCSqGSIb3DQEB
...
# AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUSTH1Cj+juudPpbfL
# zrnpJHTxtkkwXgYKKwYBBAGCNwIBDDFQME6gJoAkAE0AaQBjAHIAbwBzAG8AZgB0
# ACAATABlAGEAcgBuAGkAbgBnoSSAImh0dHA6Ly93d3cubWljcm9zb2Z0LmNvbS9s
# ZWFybmluZyAwDQYJKoZIhvcNAQEBBQAEggEASDsgR8Rscx/IIfnzI34ZXze5YAU1
# cGyxtUwJAeRLX5d2+zpcE1D1WphIB9iPs1Z1I0BlhiGb0+JX2FyX+C0TkUD6dazt
# QnQ0NIbYEigFw4irEQO7kuB531T3e1jQ3KqFn7pYveUcGVBFZI3FU4D9iTeIZklR
# IVT23ho82XMx6ky2ordGQD8PLCReIyCQkvEFcXSHUB0P1VTTlhZnjiYoyNAyix35
# W6GwoASdwRkLSgeV/lojbSPvQE8F6LawZXG+U5E1nEic/VtAroYVOSzIQAbl2Kjt
# lzz504VSaUTVkLB2tw12om2m092vk0/wnd/VEnocYvVHsSM7z/hPCYfp1qGCAh8w
...
# AQkEMRYEFIqQTxRA/h9s1C9ZXrv/1QQDuublMA0GCSqGSIb3DQEBBQUABIIBAA9c
# TPgqSU8f0+jIFvIhUg8WVk8+heKhnE72tNpLGU2dhySBklA90DUJCu85R5NFRA2T
# 6fGBDaTZ1X8PdjoJteumPXNMC4jskDcHvQfvZCL+h5YyinVS35x69HO+jlDDo4Wj
# PMIl8zglj43LI2XcCNof4K7nkrENC4DPYjLNGP31T/6vFDxeQt3z0ont2eo33a3o
# 32iVaurv2ILdq0r/1mzz6tNj3qGZSl9u01UggUOk5fnRAQ+eCsc36SKilP85rAzI
# 1zo3HLAYlCbzhgUdQaWgC6cGaFFdhW43sMmDGvhogK80y3tI2VIor9r0U4NlFtLl
# UoA2/dIaWBKVaGyqnJ4=
