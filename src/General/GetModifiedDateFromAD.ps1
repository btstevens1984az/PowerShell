# Purpose: GetModifiedDateFromAD — General-purpose PowerShell utilities.
...
function funHelp()
{
 $descriptionText= `
@"
 NAME: GetModifiedDateFromAD.ps1
 DESCRIPTION:
 This script will list the last modified date of a 
 specific user onto a local or remote domain. The
 script will allow multiple users to be supplied 
 for the -users parameter. All users who have been modified
 will have their last modified dates displayed if the
 script is run with the -all parameter. This script 
 supports prototyping by using the -whatif switch. 

 PARAMETERS: 
 -domain the domain to query for user information
 -users the user or users to query for lastlogon time.
...
@"
 ...
 SYNTAX:
 GetModifiedDateFromAD.ps1 

 Sample on a local machine

 GetModifiedDateFromAD.ps1  -all

 Displays all users last modified value from the current
 domain.  

 GetModifiedDateFromAD.ps1 -all -domain "dc=nwtraders,dc=com"

 Displays all users last modified value from the nwtraders.com
 domain. The quotation marks are required when using this syntax

 GetModifiedDateFromAD.ps1 -users mytestuser -domain 229.39.80.135

 Displays last modified value for the user named mytestuser from the
 nwtraders.com domain. The IP address 229.39.80.135 resolves to one of
 the Domain Controlers

 GetModifiedDateFromAD.ps1 -users mytestuser, auser -domain nwtraders

 Displays last modified value for the user named mytestuser, and the user
 named auser from the nwtraders.com domain. The nwtraders name resolves
 to nwtraders.com

 GetModifiedDateFromAD.ps1 -users mytestuser, auser -domain berlin

 Displays last modified value for the user named mytestuser, and the user
 named auser from the nwtraders domain. It queries directly to the 
 Domain Controller named berlin

 GetModifiedDateFromAD.ps1 -users mytestuser -domain nwtraders.com -whatif
 
 Displays what if: Perform operation obtain last modified time for user
 mytestuser from the nwtraders.com domain

 GetModifiedDateFromAD.ps1 -help

 Prints the help topic for the script

 GetModifiedDateFromAD.ps1 -help -full

 Prints full help topic for the script

 GetModifiedDateFromAD.ps1 -help -examples

 Prints only the examples for the script

 GetModifiedDateFromAD.ps1 -examples
...
" #end remarks
  ...
  if($help)     { $descriptionText; $remarks ; exit } 
...
Function funWhatIf()
{
 if($all)
  { 
   "what if: Perform operation obtain last modified time for ALL users 
   from the $domain domain"
  }
 Else
  {
   "what if: Perform operation obtain last modified time for user $users 
    from the $domain domain"
  }
 exit
} #end funWhatIf
   ...
   foreach($user in $users)
    {
     if($user.properties.item("$attribute") -ne 0)
       {
        $attributeValue = ($user.properties.item($attribute))
        "$($user.properties.item(`"name`")) was last modified $attributeValue"
       } #end if user
    } #end foreach user
 exit
} #end funGetAllUsers
 ...
 ForEach($suser in $users)
  {
   ...
   $colusers = $searcher.findall()
     foreach($user in $colUsers)
      {
       if($user.properties.item("$attribute"))
       {
        $attributeValue = ($user.properties.item($attribute))     
        "$($user.properties.item(`"name`")) was last modified $attributeValue" 
       } #end if user
       else { "can not find $user $attribute" }
      } #end foreach user
  } #end foreach suser
 exit
} #end funGetUsers
...
if(!$all -or !$users) { funhelp }


# SIG # Begin signature block
# MIIkCgYJKoZIhvcNAQcCoIIj+zCCI/cCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUOIUCYOj8niqgU7VqBbQTHP+J
# iFOggh7hMIIEEjCCAvqgAwIBAgIPAMEAizw8iBHRPvZj7N9AMA0GCSqGSIb3DQEB
...
# AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUjJ8vjYxTFMgqLVhW
# U7RnVFjgfTUwXgYKKwYBBAGCNwIBDDFQME6gJoAkAE0AaQBjAHIAbwBzAG8AZgB0
# ACAATABlAGEAcgBuAGkAbgBnoSSAImh0dHA6Ly93d3cubWljcm9zb2Z0LmNvbS9s
# ZWFybmluZyAwDQYJKoZIhvcNAQEBBQAEggEAbN2vP8G8+HaowrehBVUlSQOygfyR
# 6m+bFJdDnGKqPyHwSOylR3iFoFxu3ReOqyQYlydaED5ZfrpvnlHntV1PoIo+ohwT
# pbhIsa54mV5yqxkOItXJPYS8LYbJg9cM/dz7zjDBsNI406M9bdryBXYLahc9uP6S
# qM9BsJdZs/5gLIz4g581gonjy10gmgVIUwaBTLS6clUb1X+j58E/he6T57OxRq7m
# TJgUYLta1KhD2WPbdRFBuGzhxQ9Eo/xG3BcJMNfdTUxZLidQuIpMiEMra4jgjujb
# Cc9iRdkVh8tIdPPb3DQ+L5AO+gja0zfRpWZK+0BruJ/paMkhmYVuZkqDaqGCAh8w
...
# hkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTA4MDIwODIwMDQwNFowIwYJKoZIhvcN
# AQkEMRYEFLCqO+2XrNidJHTShjHwnEQPE0oMMA0GCSqGSIb3DQEBBQUABIIBAHqj
# uAK0ROWCXtcdXhAVjgBynbVxi+kz0fDg3KhK/GM6tMOiiIvCQlmIXXQeLL2CdAzy
# pxMouDpfEL9S/okOMRWhA7lN4nKn7oGJu6tZxCk9r6l5af6YxepGVJB4ScJOYqZl
# lXkMExP6RIWOUlMZewUiqtCtOUMBxLSTIS1nJRxyTkzbO2JJ0GJdPoIPOHYlbLuL
# Hp4ogIS0PIaKjHNLAD2UdpRipkA7y1qenIrOjX3S/lfUpkuaP/Yk3ybGy6jw6Xrg
# bWdP1CsKnSXL9q3XQUG+QPqgngIZSIHugDICryrFCGSbkArTHpzObx+1HZgBkhTK
# 8ox3yYr2xaVIVDhqMtI=
