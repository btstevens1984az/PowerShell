# Purpose: simplefunctions — General-purpose PowerShell utilities.
﻿function GetOS
{
...
function GetOS2
{
    [cmdletbinding(ConfirmImpact="low",SupportsShouldProcess=$true)]
     param ($computer= "localhost")
     if ($pscmdlet.shouldprocess($computer,"GetOS"))
     {
        (Get-WmiObject -Class Win32_OperatingSystem -ComputerName $computer).caption
     }
     write-verbose "$($pscmdlet | gm)"
     Write-verbose "$($pscmdlet | fl *)"
}
function GetOS3
{
    [cmdletbinding(ConfirmImpact="low",SupportsShouldProcess=$true)]
     ...
     if ($pscmdlet.shouldprocess($computer,"GetOS"))
     {
        ...
        (Get-WmiObject -Class Win32_OperatingSystem -ComputerName $computer -ErrorAction SilentlyContinue -ErrorVariable myerror).caption
        if ($myerror) {Write-Error $error}
     }
     write-debug "$($pscmdlet | gm)"
     Write-verbose "$($pscmdlet | fl *)"
...
<#Function Get-OS
{
    ...
    Process
    {
		...
		Select-Object @{label="ComputerName";Expression={$_.__server}},@{label="OperatingSystem";Expression={$_.caption}}        
    }
}
#>
function Get-OS {
	...
	Process
    {
        Foreach ($comp in $computer) #allows for input if its not piped into the function
        {
		    ...
		    Select-Object @{label="ComputerName";Expression={$comp}},@{label="OperatingSystem";Expression={$_.caption}}        
