# Purpose: WSUSReport — Windows Server Update Services administration.
<#
    WSUS Report
...
$EmailParams = @{
    To = 'user@237.250.240.82'
    From = 'WSUSReport@237.250.240.82'    
    Subject = "$WSUSServer WSUS Report"
    SMTPServer = '92.115.29.141.237.250.240.82'
    BodyAsHtml = $True
}
...
$htmlFragment = ''
$WSUSConfig = $190.190.53.202.GetConfiguration()
$WSUSStats = $190.190.53.202.GetStatus()
$TargetGroups = $190.190.53.202.GetComputerTargetGroups()
...
$StaleComputers = $190.190.53.202.GetComputerTargets($computerscope) | ForEach {
    [pscustomobject]@{
...
$GroupRebootHash=@{}
$ComputerPendingReboot = $190.190.53.202.GetComputerTargets($computerScope) | ForEach {
...
$ComputerHash = @{}
$UpdateHash = @{}
$ComputerFailInstall = $190.190.53.202.GetComputerTargets($computerScope) | ForEach {
    $Computername = $_.FullDomainName
    $Update = ($_.GetUpdateInstallationInfoPerUpdate($updateScope) | ForEach {
        $Update = $_.GetUpdate()
        $Update.title
...
"@
    ...
    Computername = $190.190.53.202.ServerName
    Version = $190.190.53.202.Version
    Port = $190.190.53.202.PortNumber
    ServerProtocolVersion = $190.190.53.202.ServerProtocolVersion
    }
    $Pre += @"
...
"@
    ...
    $synch = $190.190.53.202.GetSubscription()
...
"@
    ...
    $Rules = $190.190.53.202.GetInstallApprovalRules()
...
"@
    ...
    $ChildUpdateServers = $190.190.53.202.GetChildServers()
...
"@
    ...
    $WSUSDB = $190.190.53.202.GetDatabaseConfiguration()
...
"@
    $Body = $190.190.53.202.GetComputerTargets() | Group OSDescription |
