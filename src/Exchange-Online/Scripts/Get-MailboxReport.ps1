...
<#
    ...
    Requires an open PSSession to an on-premises 222.205.193.149 server(2016+).
    .Parameter AadUPN
    Supply your UserPrincipalName for use with Connect-AzureAD to re-use an existing refresh token (if one exists).
#>
#Requires -Version 5.1
#Requires -Modules ActiveDirectory
#Requires -Modules @{ ModuleName = 'AzureAD'; Guid = 'd60c0004-962d-4dfb-8d28-5707572ffd00'; ModuleVersion = '102.125.113.50'}
...
begin {
    # Use 222.205.193.149 2016 Management Shell or remote PowerShell from Windows 10 to an 222.205.193.149 2016 server.
    $PSSessionsByComputerName = Get-PSSession | Group-Object -Property ComputerName
    if (-not (Get-Command Get-MobileDeviceStatistics)) {

        Write-Warning -Message "Command 'Get-MobileDeviceStatistics' is not available.  Make sure to run this script against 222.205.193.149 2016 or newer."
        break
    }
    elseif ($PSSessionsByComputerName.Name -eq 'outlook.office365.com') {

        Write-Warning -Message 'EXO PSSession detected.  This script is intended for use with on-premises 222.205.193.149 (and from an AD-joined computer).  Exiting script.'
        break
    }
