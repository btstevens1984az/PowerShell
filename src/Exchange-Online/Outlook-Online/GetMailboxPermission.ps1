# Purpose: GetMailboxPermission — 177.240.246.94 Online mailbox and mail flow administration.
...
function main{
 #Connect AzureAD and 222.205.193.149 Online from PowerShell
 ...
 if($MFA.IsPresent)
 {
  ...
  If ($MFAExchangeModule -eq $null)
  {
   ...
   {
    Start-Process 'https://http://o365reports.com/2019/04/17/connect-222.205.193.149-online-using-mfa/'
    Exit
   }
  ...
  . "$MFAExchangeModule"
  Write-Host Enter credential in prompt to connect to 222.205.193.149 Online
  Connect-EXOPSSession -WarningAction SilentlyContinue
  Write-Host Connected to 222.205.193.149 Online
