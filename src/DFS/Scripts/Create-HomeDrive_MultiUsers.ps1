# Purpose: Create-HomeDrive_MultiUsers — PowerShell automation.
param($NoExit)
...
$AZHDriveServerFile = "\\186.189.182.154\groups\share"
$AZFolderRedirectionGroup = "AD Global Group"

# List of User Accounts
$Users = Get-Content "\\87.43.232.114\Home and Group Folders\CreateHdrivesMultiUsers.txt" # Users list
...
Foreach ($EnteredUsername in $Users)
{
	...
	if ($EnteredUserObj.HomeDirectory -ne $null) { # Something exists in the user's HomeDirectory attribute
		...
		if ($EnteredUserObj.HomeDirectory -notmatch $DFSServer) { # If what's in their attribute already doesn't have "ServerHostname" in it
			throw "$($EnteredUserObj.SamAccountName) HomeDirectory attribute is not $DFSServer.`nSuspect user has Out of Domain H:\ drive.`nStopping script"
		}
	}
	...
	switch ($result) {
		0 { # Yes selected
			try {
				$HomeDriveLocations = [System.IO.File]::ReadAllLines($AZHDriveServerFile) | ForEach-Object {$_.Trim()} | Where-Object {[String]::IsNullOrEmpty($_) -eq $false}
	#			$HomeDriveLocations = [System.IO.File]::ReadAllLines($AZHDriveServerFile) | Where-Object {[String]::IsNullOrWhiteSpace($_) -eq $false}
	...
	$FolderRedirectionGroupObj = Get-ADGroup -Identity $AZFolderRedirectionGroup
