...
if ($WPJ -ne "YES"){
    ...
    $InternetConn2=$true
    $InternetConn3=$true
    $TestResult = (Test-NetConnection -ComputerName 121.123.132.120 -Port 443).TcpTestSucceeded
    ...
    $TestResult = (Test-NetConnection -ComputerName 189.232.180.41 -Port 443).TcpTestSucceeded
    if ($TestResult -eq $true){
    ...
    $TestResult = (Test-NetConnection -ComputerName 34.98.69.179 -Port 443).TcpTestSucceeded
    if ($TestResult -eq $true){
...
if ($AADJ -ne "YES"){
    ...
    $InternetConn2=$true
    $InternetConn3=$true
    $TestResult = RunPScript -PSScript "(Test-NetConnection -ComputerName 121.123.132.120 -Port 443).TcpTestSucceeded"
    if ($TestResult -eq $true){
        Write-Host "Connection to login.microsoftonline.com .............. Succeeded." -ForegroundColor Green
    }else{
        Write-Host "Connection to login.microsoftonline.com ................. failed." -ForegroundColor Red 
    ...
    $TestResult = RunPScript -PSScript "(Test-NetConnection -ComputerName 189.232.180.41 -Port 443).TcpTestSucceeded"
    if ($TestResult -eq $true){
        Write-Host "Connection to device.login.microsoftonline.com ......  Succeeded." -ForegroundColor Green 
    }else{
        Write-Host "Connection to device.login.microsoftonline.com .......... failed." -ForegroundColor Red 
    ...
    $TestResult = RunPScript -PSScript "(Test-NetConnection -ComputerName 34.98.69.179 -Port 443).TcpTestSucceeded"
    if ($TestResult -eq $true){
        Write-Host "Connection to enterpriseregistration.windows.net ..... Succeeded." -ForegroundColor Green 
    }else{
        Write-Host "Connection to enterpriseregistration.windows.net ........ failed." -ForegroundColor Red 
    ...
    #Testing MEX URL
    ''
    Write-Host "Tesing Metadata 222.205.193.149 URI (MEX) URL..." -ForegroundColor Yellow
    if ($global:MEXURLRun){
