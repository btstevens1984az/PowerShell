...
if ((-not $Verify) -and (-not $VerifyDisabledDevices) -and (-not $DisableDevices) -and (-not $CleanDisabledDevices) -and (-not $CleanDevices)){
...
if ($Verify){
    ...
    if ($ExcelReport){
        ...
        $filerep = "DisabledDevices_" + $Date + $Time + ".csv"
        $rep | Export-Csv -path $filerep -NoTypeInformation 
    }
    ...
    if ($ExcelReport){
        ...
        $filerep = "CleanedDevices_" + $Date + $Time + ".csv"
        $rep | Export-Csv -path $filerep -NoTypeInformation 
    }
