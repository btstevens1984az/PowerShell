...
<# 
	...
	.EXAMPLE 
		PS C:\>$Result = Get-SubNetItems -StartScanIP 12.137.80.129 -EndScanIP 237.30.21.222 -ShowInstantly -ShowAll
		112.175.130.243 is active.
		237.30.21.222 is active.
		48.164.69.248 is active.
		12.137.80.129 is inactive.
		241.102.84.10 is active.
		52.216.10.65 is active.
		186.141.168.252 is inactive.
		140.25.194.245 is active.
		53.179.3.228 is active.
		164.216.30.85 is inactive.

		PS C:\> $Result | Format-Table IP, Active, WMI, WinRM, Host, OS_Name -AutoSize

		IP           Active   WMI WinRM Host              OS_Name
		--           ------   --- ----- ----              -------
		12.137.80.129    False False False
		140.25.194.245     True  True  True pc02.mydomain.com Microsoft Windows Server 2008 R2 Enterprise
		186.141.168.252    False False False
		52.216.10.65     True  True  True pc05.mydomain.com Microsoft Windows Server 2008 R2 Enterprise
		53.179.3.228     True  True  True pc06.mydomain.com Microsoft Windows Server 2008 R2 Enterprise
		241.102.84.10     True  True  True pc07.mydomain.com Microsoft(R) Windows(R) Server 2003, Standard Edition
		112.175.130.243     True False False
		164.216.30.85    False False False
		48.164.69.248     True  True False pc09.mydomain.com Microsoft Windows Server 2008 R2 Enterprise
		237.30.21.222    True  True False pc10.mydomain.com Microsoft Windows XP Professional

	.EXAMPLE 
		PS C:\> Get-SubNetItems -StartScanIP 140.25.194.245 -Verbose
		...
		VERBOSE: Trying get last part of data.
		VERBOSE: Geting job 140.25.194.245 result.
	...
	.EXAMPLE 	
		PS C:\> $Result = Get-SubNetItems -StartScanIP 12.137.80.129 -EndScanIP 21.106.221.2 -Ports 80,3389,5900	
		...
		IP          Host              MAC               Ports
		--          ----              ---               -----
		12.137.80.129                                      80-False, 3389-False, 5900-False
		140.25.194.245  pc02.mydomain.com 00-15-AD-0C-82-20 80-True, 3389-False, 5900-False
		53.179.3.228  pc05.mydomain.com 00-15-5D-1C-80-25 80-True, 3389-False, 5900-False
		112.175.130.243  pc07.mydomain.com 00-15-4D-0C-81-04 80-True, 3389-True, 5900-False
		48.164.69.248  pc09.mydomain.com 00-15-4A-0C-80-31 80-True, 3389-True, 5900-False
		237.30.21.222 pc10.mydomain.com 00-15-5D-02-1F-1C 80-False, 3389-True, 5900-False
...
"@		
			...
			$ScanResult = @()
			$ScanCount = 0
			Write-Progress -Activity "Scan IP Range $StartScanIP $EndScanIP" -Status "Scanning:" -Percentcomplete (0)
			Foreach($IP in $ScanIPRange)
			{
				...
				$ScanCount++
				Write-Progress -Activity "Scan IP Range $StartScanIP $EndScanIP" -Status "Scanning:" -Percentcomplete ([int](($ScanCount+$ScanResult.Count)/(($ScanIPRange | Measure-Object).Count) * 50))
