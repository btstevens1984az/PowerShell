# Purpose: EnableDisableUserSetPassword — Security auditing and compliance checks.
...
function funHelp()
{
 $descriptionText= `
@"
 NAME: EnableDisableUserSetPassword.ps1
 DESCRIPTION:
 Enables or Disables a user and sets the password.  
 This script supports prototyping by using
 the -whatif switch. 

 PARAMETERS: 
 -user user to enable or disable ex: cn=myuser
 -ou ou containing user ex: ou=myou
 -domain domain containing user ex:dc=nwtraders,dc=com
 -password password for user
 -enable enable the suer
...
@"
 ...
 SYNTAX:
 EnableDisableUserSetPassword.ps1 

 Displays an error missing parameter, and calls help

 EnableDisableUserSetPassword.ps1  -user "cn=myuser" -ou "ou=myou" 
 -domain "dc=nwtraders,dc=com" -disable

 Disables the user myuser in the myou in the nwtraders.com domain 

 EnableDisableUserSetPassword.ps1 -user "cn=mynewuser" -ou "ou=mytestou"
 -domain "dc=nwtraders,dc=com" -enable -password "P@ssword1!"
 
 Enables the user myuser in the myou ou in the nwtraders.com domain.
 Sets the password to P@ssword1!
 ...
 EnableDisableUserSetPassword.ps1  -user "cn=myuser" -ou "ou=myou" 
 -domain "dc=nwtraders,dc=com" -disable -whatif

 Displays message: what if: Perform operation disable myuser in ou=myou
 in dc=nwtraders,dc=com domain

 EnableDisableUserSetPassword.ps1 -help

 Prints the help topic for the script

 EnableDisableUserSetPassword.ps1 -help -full

 Prints full help topic for the script

 EnableDisableUserSetPassword.ps1 -help -examples

 Prints only the examples for the script

 EnableDisableUserSetPassword.ps1 -examples
...
Function funWhatIf()
{
 if($enable)
  {
   "what if: Perform operation enable $user in $ou ou in $domain domain"
  }
 if($disable)
  {
   "what if: Perform operation disable $user in $ou ou in $domain domain"
  }
 exit
} #end funWhatIf
...
Function funEnable()
{
  $oUser = [ADSI]"LDAP://$user,$ou,$domain"
  if($($oUser.psbase.invokeGet("AccountDisabled")))
   {
...
Function funDisable()
{
 $oUser = [ADSI]"LDAP://$user,$ou,$domain"
  if($oUser.psbase.invokeGet("AccountDisabled"))
   {
    funline("$user account in $ou ou in $domain domain is already disabled")
   }
  Else
   {
    $oUser.psbase.invokeSet("AccountDisabled","True")
    $oUser.psbase.CommitChanges()
   }
} #end funDisable
...
if($examples)  { funhelp }
if($full)      { funhelp }
if(!$user -or !$ou -or !$domain) { funhelp }
if($whatif)    { funWhatIf }
if($enable) { funEnable }
if($disable) { funDisable }

# SIG # Begin signature block
# MIIkCgYJKoZIhvcNAQcCoIIj+zCCI/cCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUvTNvy+qzC5tvEzmYPv9w757V
# MkGggh7hMIIEEjCCAvqgAwIBAgIPAMEAizw8iBHRPvZj7N9AMA0GCSqGSIb3DQEB
...
# AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUC6nQ3slOEL0Yp9zy
# Xm+MfSKQvOQwXgYKKwYBBAGCNwIBDDFQME6gJoAkAE0AaQBjAHIAbwBzAG8AZgB0
# ACAATABlAGEAcgBuAGkAbgBnoSSAImh0dHA6Ly93d3cubWljcm9zb2Z0LmNvbS9s
# ZWFybmluZyAwDQYJKoZIhvcNAQEBBQAEggEAV7PWM0kpNe9WmZm0jmEz+S8VfSzR
# arYi54a/t3UvHonQ4SLKuquD8pNzRHAf7wT1mmMGjcQfB398y/sWGCunYC8fvc37
# vg77AR4j3Q2JBk+z+dH1yEsc7aKWJQh8prjbwNHduVb+Dmz5zI+3aBX4OMRzHj7y
# dX0SrqhuxN7sdmFcAACy++WwW+9AgT5BMBfpbslMZLszCweQcV4dNoX4vhu18jqG
# LDKv7HRDmJZae/2ucdur/M2s/TL7tP3tGd8PdeM0eTDrfU2O+e5EP8j4gIemYUmQ
# uWN73CFG8EwO94FW4BedAVQU+46tQ2yhw3sQ3WX0v2IN/jQwqLtsXxRgq6GCAh8w
...
# hkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTA4MDIwODIwMDM1OVowIwYJKoZIhvcN
# AQkEMRYEFAFCpzx9D2J27NfTB/dV4h14uIrpMA0GCSqGSIb3DQEBBQUABIIBAGFf
# nROS/u0ze9jcTUvxA/NxztZr9GWzxGhBg1uLLMijzReDRcdWmbbz57DWPDvJPu56
# 7YcAyNlt+BpuEgMOJy7JaWCXNK2s1A7RnD+thINHcwf4LzkR8Ef7Xa+NoM0h//d9
# Z/KY1lexg08hKzMbXua+4+w0+9TWjWKtAWAhTkpXd98WXAiZGVZLFxZPw69R128N
# Qg2ZO1PETLzU7hcjxjaP+Ukoq+xUP5OL14X3povzHDoB+2U4QMqRWoup8licKAx5
# 6eBAPizFQJbpqc7vMSna+7eLIFjTduANkPQ2J9p3D/ghtHdTaDvuQLNr+L0J+B3U
# u6xgLRajgeFAKarGU2E=
