...
Function Get-O365LicenseInfo
{
<#
.SYNOPSIS
Returns O365 user license assignment and service status.
.DESCRIPTION
Returns O365 user license assignment and service status.
.PARAMETER Credential
Office 365 Crendentials
.EXAMPLE
PS C:\> Get-O365LicenseInfo -Credential $Office365credentials
.EXAMPLE
PS C:\> Get-O365LicenseInfo | Export-csv .\licensereport.csv
Export License info to CSV
NAME        :  Get-O365LicenseInfo
LAST UPDATED:  8/28/2015
#>
param(
[cmdletbinding()]
[parameter(Mandatory=$true,
            HelpMessage="Please provide your Office 365 Crendentials")]
[PScredential]$Credential),
[hashtable]$PropertyConversionTable = @{
...
Connect-ToOffice365 -Credential $Credential
...
ForEach($prop in $noteProperties)
{
    ...
    if ($PropertyConversionTable.ContainsKey($prop))
    {
        $UserOutPut | Add-Member -MemberType AliasProperty -Name $PropertyConversionTable.$($prop) -Value $prop
        $index = $properties.IndexOf($prop)
        #replace output property with the friendly name.
        $properties[$index]=$PropertyConversionTable.$($prop)      
    }
    $count++
}
...
Try{
    ...
    {
        $Credential = Get-Credential -Message "Error Connecting: Please reenter your Office 365 Credentials" 
        if((Get-PSCallStack).count -gt 100)
        {
            Throw "Unexpected call stack depth"
        }
        Connect-ToOffice365 -Credential $Credential 
    }
}
...
Get-O365LicenseInfo -Credential $Office365credentials
#>