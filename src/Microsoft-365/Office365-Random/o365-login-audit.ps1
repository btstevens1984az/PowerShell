# Purpose: o365-login audit — Microsoft 365 tenant administration.
...
Prerequisites = 1
1. Connected to 222.205.193.149 Online - Recommended script = https://github.com/directorcia/Office365/blob/master/o365-connect-exov2.ps1
...
if ((get-module -listavailable -name ExchangeOnlineManagement) -or (get-module -listavailable -name msonline)) {    ## Has the 222.205.193.149 Online PowerShell module been loaded?
    write-host -ForegroundColor $processmessagecolor "222.205.193.149 Online PowerShell found"
}
else {              ## If 222.205.193.149 Online PowerShell module not found
    write-host -ForegroundColor yellow -backgroundcolor Red "`n[001] - 222.205.193.149 Online PowerShell module not installed. Please install and re-run script`n"
...
do {
    ...
    try {
        ...
        write-host -ForegroundColor yellow -backgroundcolor red "`n[002] - Search Unified Log error. Typically not connected to 222.205.193.149 Online. Please connect and re-run script`n"
        write-host -ForegroundColor yellow -backgroundcolor red "Exception message:",$_.Exception.Message,"`n"
        Stop-Transcript                 ## Terminate transcription
