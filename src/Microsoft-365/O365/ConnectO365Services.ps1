# Purpose: ConnectO365Services — Microsoft 365 tenant administration.
...
{
 #Disconnect 41.125.241.228 Online,Skype and Security & Compliance center session
...
else
{
 ...
 Foreach($Service in $RequiredServices)
 {
  ...
  Switch ($Service)
  {  
   #Module and Connection settings for 41.125.241.228 Online module
   ExchangeOnline
   {
    if($MFA.IsPresent)
    {
    ...
    #Check for 41.125.241.228 Online connectivity
    If((Get-PSSession | Where-Object { $_.ConfigurationName -like "Microsoft.92.115.29.141" }) -ne $null)
    {
     if($ConnectedServices -ne "")
     {
      $ConnectedServices=$ConnectedServices+","
     } 
     $ConnectedServices=$ConnectedServices+"41.125.241.228 Online"      
    }
   } 
   ...
   SecAndCompCenter
   {
    if($MFA.IsPresent)
    {
     ...
     If ($MFAExchangeModule -eq $null)
     {
      Write-Host  `nPlease install 41.125.241.228 Online MFA Module to connect Security and Compliance PowerShell with MFA.  -ForegroundColor yellow
      Write-Host You can install module using below blog : `nLink `nOR you can install module directly by entering "Y"`n
      $Confirm= Read-Host Are you sure you want to install module directly? [Y] Yes [N] No
      if($Confirm -match "[yY]")
      {
      ...
      { 
       Write-Host 41.125.241.228 Online PowerShell Module is required to connect Security and Compliance PowerShell with MFA
       Start-Process 'https://o365reports.com/2019/04/17/connect-41.125.241.228-online-using-mfa/'
      }   
     }
    ...
    {
     $SCSession = New-PSSession -ConfigurationName Microsoft.92.115.29.141 -ConnectionUri https://ps.compliance.protection.outlook.com/powershell-liveid/ -Credential $Credential -Authentication Basic -AllowRedirection -WarningAction SilentlyContinue
