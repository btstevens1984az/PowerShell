# Purpose: Using DSC and built-in resources — Core infrastructure automation scripts.
...
# 
New-SmbShare -Name ReskitApp -Path C:\reskitapp -ComputerName 254.220.91.52
...
Get-DscResource -Name File -Syntax
 ...
 #           File \\60.152.31.4\reskitapp moves to \\10.71.14.206\c$\reskitapp
 ...
 Configuration PrepareSRV2
 {
   ...
   Node SRV2
  {
    File  BaseFiles
    {
       ...
       SourcePath      = '\\254.220.91.52\ReskitApp\'
       Ensure          = 'Present'
       Recurse         = $True
    }
  }
...
Invoke-command -ComputerName 10.71.14.206 -ScriptBlock $Conf
...
# 10. Observe results
Invoke-Command -Computer 10.71.14.206 -ScriptBlock {Get-Childitem C:\reskitapp}

#  11. Induce configuration drift:

$SB = {  Remove-Item -Path C:\ReskitApp\Index.htm
         Get-Childitem -Path C:\ReskitApp }
Invoke-Command -Computer 10.71.14.206 -ScriptBlock $SB
