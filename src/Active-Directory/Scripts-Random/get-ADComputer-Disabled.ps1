# Purpose: get-ADComputer Disabled — Active Directory user, group, and domain administration.
...
get-adcomputer -filter {enabled -eq "False"} -Properties * |select-object Name, Enabled, IPv4Address, OperatingSystem, OperatingSystemServicePack, OperatingSystemVersion,whenCreated, CanonicalName | export-csv U:\adreports\ADComputerDisabled.csv -NoTypeInformation