# Purpose: LocateOldComputersNotLogon — Windows desktop configuration and management.
...
function funHelp()
{
 $descriptionText= `
@"
 NAME: LocateOldComputersNotLogon.ps1 
 DESCRIPTION:
 Locates computer accounts in a local or remote
 domain that have not logged on for a specified 
 number of days. The -domain parameter controls the
 domain to query. By default the local domain is used.
 The -days parameter controls the number of days since
 logon. By default the number of day is set to 30.This 
 script supports prototyping by using the -whatif 
 switch.

 PARAMETERS: 
 -computers The computer or computers account to query
 -domain The domain to query
 -days The number of days since logon
 -all queries for all comptuer accounts in the domain
...
@"
 ...
 LocateOldComputersNotLogon.ps1   -computer 157.217.58.110

 Returns the number of days since the Berlin computer has
 logged onto the current domain. 

 LocateOldComputersNotLogon.ps1   -domain nwtraders -all

 Returns the number of days since each computer in the nwtraders
 domain has logged in

 LocateOldComputersNotLogon.ps1  -all -whatif
 
 Displays what if: Perform operation query computer accounts.

 LocateOldComputersNotLogon.ps1  -help

 Prints the help topic for the script

 LocateOldComputersNotLogon.ps1  -help -full

 Prints full help topic for the script

 LocateOldComputersNotLogon.ps1  -help -examples

 Prints only the examples for the script

 LocateOldComputersNotLogon.ps1  -examples
...
Function funWhatIf()
{
 if($all)
  {
   "what if: Perform operation query computer all accounts"
   exit
  }
 ELSE
  {
   "what if: Perform operation query computer accounts $computers"
   exit
  }
} #end funWhatIf

function funGetAllcomputers() 
{
 $attribute = "lastlogon"
 ...
 $computers = $searcher.findall()
   foreach($computer in $computers)
    {
     if($computer.properties.item("$attribute") -ne 0)
       {
        $attributeValue = [datetime]::FromFileTime([int64]::Parse($computer.properties.item($attribute)))
        "$($computer.properties.item(`"name`")) $attributeValue"
       } #end if computer
     ELSE { "$($computer.properties.item(`"name`")) has never logged on" }
    } #end foreach computer
} #end funGetAllComputers

function funGetComputers() 
{
 $attribute = "lastlogon"
 $searcher = new-object DirectoryServices.DirectorySearcher([ADSI]"$domain")
 ForEach($scomputer in $computers)
  {
   $sComputer = $sComputer +"$"
   $searcher.filter = "(sAMAccountName=$scomputer)"
   $colComputers = $searcher.findall()
   
     foreach($scomputer in $colcomputers)
      {
       if($scomputer.properties.item("$attribute") -ne 0)
       {
        $attributeValue = [datetime]::FromFileTime([int64]::Parse($scomputer.properties.item($attribute)))
        "$($scomputer.properties.item(`"name`")) $attributeValue"
       } #end if scomputer
       ELSE { "$($scomputer.properties.item(`"name`")) has never logged on" } 
      } #end foreach scomputer
  } #end foreach scomputer
} #end funGetComputers
...
if($whatif)    { funWhatIf }
if($computers)  { funGetComputers }
if($all)       { funGetAllComputers }
if(!$computers -and !$all) { funhelp }

# SIG # Begin signature block
# MIIkCgYJKoZIhvcNAQcCoIIj+zCCI/cCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUBmKafiS4stg3oWz5D+NpObFb
# Ok2ggh7hMIIEEjCCAvqgAwIBAgIPAMEAizw8iBHRPvZj7N9AMA0GCSqGSIb3DQEB
...
# AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUpI7lNSZGv9VexFh2
# Rngiy6C/Yl8wXgYKKwYBBAGCNwIBDDFQME6gJoAkAE0AaQBjAHIAbwBzAG8AZgB0
# ACAATABlAGEAcgBuAGkAbgBnoSSAImh0dHA6Ly93d3cubWljcm9zb2Z0LmNvbS9s
# ZWFybmluZyAwDQYJKoZIhvcNAQEBBQAEggEAKv8rVY67wTDJ9IslQqWmYdKwqgVE
# hGB01EHJOihg5VriHvpXcIreH9rKJeaP8yLk6CJ3XEJZExAR4Q7FBNNCTHjnynZd
# ZtCE/qSRzepqvYaFQ9DCDzKkr+j04bWPh8PxTLBV4U4WmaOQ9muDtkPa5H0/wyYr
# b6NWS7DWSpRYys2EW7+OEKFcKyaMleP3J48GL8KL3KPO5pzQIvSVBN6gz/b7zLka
# CskSJByAx81WTlan9cfCRuiCi5fCA/Qiqvpu296yw64WfZEdi80BNjdYM/LzU5a3
# GS9PJZahNT18IAeONXjLg/+dywCvigbOwyrT/e0W6rVROrKJtzUJrMT/4KGCAh8w
...
# hkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTA4MDIwODIwMDQwMlowIwYJKoZIhvcN
# AQkEMRYEFH53kYpzF9SiVr9HLJ2Lo5b4ymDaMA0GCSqGSIb3DQEBBQUABIIBAD38
# YMQM55CM7AN+r0iYPw6hH53yj+w2XVBq+iWPqUTBRTNEex5Jc4CQ1iUbumgO9UH5
# YKtNZkDY1P/HOEE9S3v6bbRAugHv+TXyuEcrz9IiIwoptbiTb3f/5Z73So17C7V8
# M008R7wwpdUqUQnwSjtxl+vPDowDirqggFlSPbVyqMF+nUmx/FhypZiEO5WfQF5L
# sZ9JNuRafUH7Vam9IhiV1FZ6sC6GMSy/y5nFqyxv1E3Cb23Tis1mb7B/ygwN1s7z
# nwXGMKrJF0CB4BRfoiCikewk0xWy9eTnC1llpH1rphcdgLVYZzofeFiLqourMS1W
# 50u6VxEdb+VNSoyNXoY=
