...
#>
Function Get-InstalledSoftwareOnComputer
{
	Param
	(
	...
	Process
	{
		ForEach($Computer in $Name)
		{
			...
			$MasterKeys = ($MasterKeys | Where {$_.Name -ne $Null -AND $_.Name -like "Google*" -OR $_.Name -like "Mozilla*" -OR $_.Name -like "Adobe*" -OR $_.Name -like "Java*"  -OR $_.Name -like "Radia*" -OR $_.Name -like "iTunes" -AND $_.SystemComponent -ne "1" -AND $_.ParentKeyName -eq $Null} | select Name,Version,ComputerName,UninstallCommand | sort Name )
			$MasterKeys 
		}
	}
	End
