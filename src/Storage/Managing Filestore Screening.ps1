# Purpose: Managing Filestore Screening — Storage management and disk operations.
...
$SB = {
  $FSTHT = @{
    Path        = "$Env:windir\notepad.exe"
    Destination = '\\60.152.31.4\screen\notepad.txt'
  }
  Copy-Item  @FSTHT
}
Invoke-command -ComputerName 60.152.31.4 -ScriptBlock $SB -Credential $Cred
