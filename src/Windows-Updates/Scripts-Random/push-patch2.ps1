# Purpose: push-patch2 — Windows Update and patch management.
#----------------------------
...
function Push-Patch2{
    ...
    Foreach($cn in $ComputerName)
    {
        ...
        if($CoreSatellite){
            Start-Process 'C:\Program Files (x86)\Hewlett-Packard\HPCA\Agent\radntfyc.exe' -ArgumentList "$cn radskman ip=214.193.3.93,port=3464,cat=prompt,ulogon=n,hreboot=n,dname=Patch,log=connect_Patch.log,rtimeout=60,rcsuri=tcp://119.223.48.126:3464,datauri=http://119.223.48.126:3466"
        }
        else{
