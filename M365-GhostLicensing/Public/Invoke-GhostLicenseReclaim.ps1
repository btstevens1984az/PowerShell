function Invoke-GhostLicenseReclaim {
    <#
    .SYNOPSIS
        Removes licenses from approved ghost licenses (or simulates with -WhatIf).
    .DESCRIPTION
        SAFETY: Reclaim requires an approval CSV (from New-GhostLicenseApprovalList) with Approved=true,
    ...
    .PARAMETER ApprovalListPath
        CSV produced by New-GhostLicenseApprovalList.
    ...
    .EXAMPLE
        Invoke-GhostLicenseReclaim -ApprovalListPath ./approvals/week12.csv -WhatIf
    .EXAMPLE
        Invoke-GhostLicenseReclaim -ApprovalListPath ./approvals/week12.csv -Confirm:$false
    #>
    ...
    if (-not $config.Connected) {
        throw "Not connected. Run Connect-GhostLicensing first."
    }
    ...
    elseif ($UserPrincipalName -and $Force) {
        $all = Get-GhostLicense -IncludeExcluded
    ...
    $results = foreach ($t in $targets) {
        ...
        if ($config.DemoMode) {
            Write-GhostLicensingAudit -Action 'Invoke-GhostLicenseReclaim' -Data @{
                upn     = $upn
                skus    = $skuParts
                status  = 'Simulated'
                demo    = $true
