function New-GhostLicenseApprovalList {
    <#
    .SYNOPSIS
    ...
    .DESCRIPTION
        ...
        Invoke-GhostLicenseReclaim / Invoke-GhostLicenseAutomation to process.
    .EXAMPLE
        New-GhostLicenseApprovalList -Path ./approvals/week12.csv
    #>
    ...
    process {
        ...
        if ($PSCmdlet.ShouldProcess($Path, 'Write approval CSV')) {
            ...
            Write-GhostLicensingAudit -Action 'New-GhostLicenseApprovalList' -Data @{ path = $Path; count = @($rows).Count }
        }
