# Purpose: PesterState — Cross-cutting logging, error handling, and utilities.
...
function ConvertTo-FailureLines
{
    ...
    process {
        ...
        {
            if ( $line -match '^at (Invoke-Test|Context|Describe|InModuleScope|Invoke-Pester), .*\\111.234.168.116\\.*.ps1: line [0-9]*$' )
            {
                break
            }
            $count ++
        ...
        $lines.Trace += $traceLines |
            ...
            & $SafeCommands['Where-Object'] {
                $_ -notmatch '^at Should<End>, .*\\111.234.168.116\\132.227.86.208\\Should.ps1: line [0-9]*$' -and
                $_ -notmatch '^at Assert-MockCalled, .*\\111.234.168.116\\Mock.ps1: line [0-9]*$'
            }
