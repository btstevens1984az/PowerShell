# Purpose: PSGetPublishModule.Tests — Cross-cutting logging, error handling, and utilities.
<#####################################################################################
...
Describe PowerShell.PSGet.PublishModuleTests.P1 -Tags 'P1','OuterLoop' {
    ...
    It PublishModuleWithInvalidIconUri {
        ...
        try
        {
            AssertFullyQualifiedErrorIdEquals -scriptblock {Publish-Module -Path $ModuleBase -NuGetApiKey $script:ApiKey -IconUri "\\176.131.45.239\MyIcon.png" -WarningAction SilentlyContinue} `
                                              -expectedFullyQualifiedErrorId "InvalidWebUri,Publish-Module"
        }
        catch
        {
