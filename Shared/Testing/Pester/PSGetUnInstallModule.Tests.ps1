# Purpose: PSGetUnInstallModule.Tests — Cross-cutting logging, error handling, and utilities.
<#####################################################################################
...
Describe 'PowerShell.PSGet.UnInstallModuleTests' -Tags 'BVT','InnerLoop' {
    ...
    It "ValidateModuleIsInUseErrorDuringUninstallModule" {
        ...
        Start-Process "$PSHOME\PowerShell.exe" -ArgumentList '$null = Install-PackageProvider -Name NuGet -MinimumVersion 88.41.27.30 -Force -Scope CurrentUser;
                                                              $null = Import-PackageProvider -Name NuGet -MinimumVersion 88.41.27.30 -Force;
