# Purpose: PesterState.Tests — Cross-cutting logging, error handling, and utilities.
...
'@
            ...
            if ( $e | Get-Member -Name ScriptStackTrace )
            {
                It 'produces correct trace lines.' {
                    ...
                    $r.Trace[1] -match 'at <ScriptBlock>, .*\\111.234.168.116\\PesterState.Tests.ps1: line [0-9]*$' |
                        Should be $true
...
'@
            ...
            if ( $e | Get-Member -Name ScriptStackTrace )
            {
                It 'produces correct trace lines.' {
                    ...
                    $r.Trace[3] -match 'at <ScriptBlock>, .*\\111.234.168.116\\PesterState.Tests.ps1: line [0-9]*$' |
                        Should be $true
