function Export-GhostSeatReport {
    <#
    .SYNOPSIS
    ...
    process {
        ...
        $arr = @($seats.ToArray())
        $summary = $arr | Get-GhostSeatSummary

        if (-not $Path) {
            $Path = Join-Path $config.DefaultReportPath ("GhostSeats-{0:yyyyMMdd-HHmmss}" -f [datetime]::UtcNow)
        }

        $baseDir = $Path
        $baseName = 'GhostSeats-Report'
        ...
        if ('Html' -in $Format) {
            ...
            New-GhostSeatHtmlReport -Seats $arr -Summary $summary -Path $htmlPath | Out-Null
        ...
        Write-GhostSeatAudit -Action 'Export-GhostSeatReport' -Data @{ outputs = $outputs; count = $arr.Count }
