function Get-GhostSeat {
    <#
    .SYNOPSIS
        Finds Microsoft 365 ghost seats (licensed users that look unused).
    .DESCRIPTION
        Returns GhostSeat objects for:
    ...
    .EXAMPLE
        Get-GhostSeat -Category DisabledAccount, NeverSignedIn | Export-Csv .\ghosts.csv
    #>
    [CmdletBinding()]
    [Alias('ggs')]
    param(
    ...
    if (-not $config.Connected) {
        throw "Not connected. Run Connect-GhostSeats -Demo or Connect-GhostSeats first."
    }
    ...
    $users = @(Get-GhostSeatUserInventory)
    ...
    $results = foreach ($user in $users) {
        ...
        $excluded = Test-GhostSeatExcluded -User $user
        ...
        $seat = New-GhostSeatObject -User $user -Category $cat -Licenses $user.AssignedLicenses -LastSignIn $last -InactiveDays $inactiveDaysValue -Reason $reason
        if ($excluded) {
    ...
    Write-GhostSeatAudit -Action 'Get-GhostSeat' -Data @{
        count         = @($results).Count
