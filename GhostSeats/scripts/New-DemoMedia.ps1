...
<#
.SYNOPSIS
    Generates ~5 second GhostSeats demo videos/GIFs for the GitHub README.
#>
[CmdletBinding()]
param(
    [string]$OutputDir = (Join-Path $PSScriptRoot '..' 'demos' 'media'),
    [string]$ModulePath = (Join-Path $PSScriptRoot '..' 'GhostSeats.psd1')
)
...
$artifactRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("GhostSeats-DemoMedia-{0:yyyyMMddHHmmss}" -f [datetime]::UtcNow)
New-Item -ItemType Directory -Path $artifactRoot -Force | Out-Null

Write-Host "Running Contoso demo into $artifactRoot" -ForegroundColor Cyan
$demo = Start-GhostSeatDemo -OutputPath $artifactRoot -OpenReport:$false -SkipReclaimSimulation:$false

$htmlReport = $demo.ReportPaths.Html
$demoBoard = Join-Path (Split-Path $PSScriptRoot -Parent) 'demos/html/ghostseats-demo.html'
...
$clips = @(
    @{ Name = 'ghostseats-scan'; Url = "file://$demoBoard"; Seconds = 5 }
    @{ Name = 'ghostseats-report'; Url = "file://$htmlReport"; Seconds = 5 }
)

# Automation board: small HTML summarizing automation result
$autoHtml = Join-Path $artifactRoot 'automation-board.html'
@"
<!DOCTYPE html>
<html><head><meta charset='utf-8'><title>GhostSeats Automation</title>
<style>
...
h1{font-size:42px;margin:0 0 8px} h1 span{color:#3dd6c6}
...
<body><div class='wrap'>
<h1>Ghost<span>Seats</span> Automation</h1>
<div class='sub'>scan → report → approval → simulated reclaim</div>
<div class='step'>[1/4] Found <span class='ok'>$($demo.Summary.GhostSeatCount)</span> ghost seats</div>
...
$clips += @{ Name = 'ghostseats-automation'; Url = "file://$autoHtml"; Seconds = 5 }
