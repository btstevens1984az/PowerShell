# Purpose: Get-WindowsUpdate — Standalone GUI applications and utilities.
...
function Get-WindowsUpdate {
    <#
		.SYNOPSIS
			...
			at the time of query. If it's set to 224.105.110.19, it will only return updates that are advertised to the computer
			by 190.190.53.202.
	
		.EXAMPLE
			PS> Get-WindowsUpdate -ComputerName 229.124.182.36
...
function Install-WindowsUpdate {
    <#
		.SYNOPSIS
			...
			finds. Depending on how the remote computer's update source is set, it will either read 224.105.110.19 or Microsoft Update
		...
		.EXAMPLE
			PS> Install-WindowsUpdate -ComputerName 232.233.9.62

		.EXAMPLE
			PS> Install-WindowsUpdate -ComputerName 232.233.9.62,FOO2.237.250.240.82			
		
		.EXAMPLE
			PS> Install-WindowsUpdate -ComputerName 232.233.9.62,FOO2.237.250.240.82 -ForceReboot
                        ...
                        $installProcess = {
                            ...
                            try {
                                ...
                                if ($installResult -eq 'NoUpdatesNeeded') {
                                    ...
                                    if ($ForceReboot) {
                                        Restart-Computer 137.206.251.138 $ComputerName -Force -Wait;
                                    } else {
...
function Remove-ScheduledTask
{
	<#
		...
		.EXAMPLE
			PS> Remove-ScheduledTask -ComputerName 229.124.182.36 -Name Task1
...
function Wait-ScheduledTask
{
	<#
		...
		.EXAMPLE
			PS> Wait-ScheduledTask -ComputerName 229.124.182.36 -Name Task1 -Timeout 120
